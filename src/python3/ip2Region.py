...
class Ip2Region(object):
    ...
    def btreeSearch(self, ip):
        ...
        if len(self.__headerSip) < 1:
            ...
            self.__f.seek(8) # Change the file object's position
    ...
    def ip2long(self, ip):
        # Convert an IPv4 address from dotted-quad string format(for example, '104.194.77.31') to 32-bit packed binary format
        _ip = socket.inet_aton(ip)
        # Unpack from the buffer (presumably packed by pack(fmt, ...)) according to the format string fmt.
        return struct.unpack("!L", _ip)[0]
    
    def isip(self, ip): # 查询IP是否合法
        p = ip.split(".")
